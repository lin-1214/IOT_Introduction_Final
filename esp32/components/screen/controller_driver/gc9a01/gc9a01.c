...
esp_err_t lcd_gc9a01_init(const scr_controller_config_t *lcd_conf) {
    LCD_CHECK(lcd_conf->width <= GC9A01_RESOLUTION_HOR, "Width greater than maximum",
              ESP_ERR_INVALID_ARG);
    LCD_CHECK(lcd_conf->height <= GC9A01_RESOLUTION_VER, "Height greater than maximum",
              ESP_ERR_INVALID_ARG);
    LCD_CHECK(NULL != lcd_conf, "config pointer invalid", ESP_ERR_INVALID_ARG);
    LCD_CHECK(
        (NULL != lcd_conf->interface_drv->write_cmd &&
         NULL != lcd_conf->interface_drv->write_data && NULL != lcd_conf->interface_drv->write &&
         NULL != lcd_conf->interface_drv->read && NULL != lcd_conf->interface_drv->bus_acquire &&
...
esp_err_t lcd_gc9a01_set_window(uint16_t x0, uint16_t y0, uint16_t x1, uint16_t y1) {
    LCD_CHECK((x1 < g_lcd_handle.width) && (y1 < g_lcd_handle.height),
              "The set coordinates exceed the screen size", ESP_ERR_INVALID_ARG);
    LCD_CHECK((x0 <= x1) && (y0 <= y1), "Window coordinates invalid", ESP_ERR_INVALID_ARG);
    esp_err_t ret = ESP_OK;
    scr_utility_apply_offset(&g_lcd_handle, GC9A01_RESOLUTION_HOR, GC9A01_RESOLUTION_VER, &x0, &y0,
                             &x1, &y1);
